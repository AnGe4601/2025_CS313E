...
class Graph:
    ...
    def compute_depth(self):
        ...
        def dfs_depth(row_index):
            # base case where vertex has been visited already
            if self.vertices[row_index].depth != -1:
                return self.vertices[row_index].depth
            # get its adjacent vertices; return column_index
            max_depth = 0
            adj_vertices = self.get_adjacent_vertices(row_index)
            # traverse its neighbor
            for adj_vertex_index in adj_vertices:
                # traverse its other neighbors and find the longest path
                max_depth = max(max_depth, dfs_depth(adj_vertex_index) + 1)
            self.vertices[row_index].depth = max_depth
            return max_depth
        # traverse all vertices
        for row_index, vertex in enumerate(self.vertices):
            # haven't visit
            if vertex.depth == -1:
                # call recursion to check its children
                dfs_depth(row_index)
    ...
    def get_registration_plan(self):
        ...
        for vertex in self.vertices:
            prereque[vertex.label] = 0
        class_taken = set()

        # filling prereque
        for row, vertex in enumerate(self.vertices):
            for column in range(len(self.adjacency_matrix[row])):
                # has an incoming edge -> has prereque
                if self.adjacency_matrix[row][column] == 1:
                    prereque[self.vertices[column].label] += 1
        # while there are avilable class to take
        while len(class_taken) < len(self.vertices):
            semester = []
            # if vertex has 0 prereque then we insert
            for vertex in self.vertices:
                if prereque[vertex.label] == 0:
                # then use bh to determine the longest path
                    bh.insert((-vertex.depth, vertex.label))
            while not bh.is_empty() and len(semester) < 4:
                curr_class = bh.delete()
                if not curr_class in class_taken:
                    semester.append(curr_class[1])
                    class_taken.add(curr_class)
                    # update children's prereque
                    adj_vertice_index = self.get_adjacent_vertices(self.get_index(curr_class[1]))
                    for child_index in adj_vertice_index:
                        child_vertex = self.vertices[child_index]
                        prereque[child_vertex.label] -= 1
...
def main():
    ...
    for edge in range(num_edges):
        edge = input().split()
        edge_start = graph.get_index(edge[0])
        edge_finish = graph.get_index(edge[1])
        graph.add_edge(edge_start, edge_finish)
    ####################################################################################
