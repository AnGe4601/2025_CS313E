...
def prepare_game():
    ...
    with open("secret_words.txt", "r", encoding="ascii") as valid_secret_words:
        valid_sec_words = [word.rstrip() for word in valid_secret_words.readlines()]
    secret_word = ""

    # nothing in command than randomly chose one
    if len(sys.argv) == 1:
        secret_word = random.choice(valid_sec_words)
    elif len(sys.argv) == 2:
        ...
        if sys.argv[1].isnumeric():
            ...
            secret_word = random.choice(valid_sec_words)
        # string
        elif sys.argv[1].isalpha() and len(sys.argv[1]) == 5 and sys.argv[1].islower():
